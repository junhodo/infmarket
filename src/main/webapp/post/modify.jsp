...
<section id="contentSection" class="col-sm-10 d-inline-block float-left infSectionMinHeight shadow p-3 bg-white rounded">
    <div class="input-group input-group-lg mb-lg-5">
        ...
        <input type="text" id="post_title" class="form-control" aria-label="Large" aria-describedby="inputGroup-sizing-sm" placeholder="5자 이상 작성해주세요" value=${title}>
    </div>
    <div id="text" aria-placeholder="5자 이상 작성해주세요">${text}</div>
    <button id="update" class="float-right btn btn-dark mt-3">수정</button>
</section>
</body>
</html>
...
<script>
    ...
    $("#update").click(function(){
        ...
        $.ajax({
            url  : '/api/post/update',
            type : 'post',
            data : {postId : ${postId},title: $("#post_title").val(), text: $('#text').summernote('code')},
            success: function(xhr, textStatus, error){
                alert("성공적으로 수정되었습니다")
                window.location.replace("/freeboard/1")
            },
            error: function(xhr, textStatus, error){
                alert("수정에 실패했습니다, 제목과 내용의 길이를 확인해주세요")
            }
        })
    });
</script>