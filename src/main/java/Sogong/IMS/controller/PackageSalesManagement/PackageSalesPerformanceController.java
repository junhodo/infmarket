...
@WebServlet("/packageSalesPerformance/*")
public class PackageSalesPerformanceController extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(request.getSession().getAttribute("member") != null && hasAuthority((Member)request.getSession().getAttribute("member"),"상품_집계")) {
    ...
    public void init(ServletConfig config) throws ServletException {
        list = new HashMap<>();
        list.put("lookup.do", new PackageSalesPerformanceLookupAction());
    }
